...
public class WorldGenWaterPlant extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
	    ...
	    for (int l = 0; l < 64; ++l) {
	        ...
	        if (submerged && (!world.provider.hasSkyLight() || py < 254) && world.getBlockState(pPos.add(0, -1, 0)).getBlock() == ModBlocksSpace.laythe_silt) {
	            
	            if (rand.nextBoolean()) {
					//world.setBlockState(pos, ModBlocksSpace.plant_tall_laythe, 2);
	                //world.setBlockState(pos.add(0, 1, 0), ModBlocksSpace.plant_tall_laythe, 2);
	            } else {
	                int height = 2 + rand.nextInt(4); 
	                if(py <= 57) {
		                for (int h = 0; h < height; ++h) {
		                    //world.setBlockState(pos.add(0, h, 0), ModBlocksSpace.laythe_kelp, 1);
		                }	                	
	                }
	            ...
	            if (world.getBlockState(new BlockPos(px2, py2 - 1, pz2)).getBlock() == ModBlocksSpace.laythe_silt) {
	                //world.setBlockState(new BlockPos(px2, py2, pz2), ModBlocksSpace.laythe_short, 2);
	            }
